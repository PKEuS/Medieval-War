...
macro(sfphysics_static_add_libraries target)
...
macro(sfphysics_list_contains var value)
    set(${var})
    foreach(value2 ${ARGN})
...
# ex: sfphysics_parse_arguments(THIS "SOURCES;DEPENDS" "FLAG" FLAG SOURCES s1 s2 s3 DEPENDS d1 d2)
# will define the following variables:
# - THIS_SOURCES (s1 s2 s3)
# - THIS_DEPENDS (d1 d2)
# - THIS_FLAG TRUE
macro(sfphysics_parse_arguments prefix arg_names option_names)
    ...
    foreach(arg ${ARGN})
        sfphysics_list_contains(is_arg_name ${arg} ${arg_names})
        if(is_arg_name)
            set(${prefix}_${current_arg_name} ${current_arg_list})
            set(current_arg_name ${arg})
            set(current_arg_list)
        else()
            sfphysics_list_contains(is_option ${arg} ${option_names})
            if(is_option)
...
macro(sfphysics_add_library target)
    ...
    # parse the arguments
    sfphysics_parse_arguments(THIS "SOURCES;DEPENDS;EXTERNAL_LIBS" "" ${ARGN})
    ...
    # link the target to its sfphysics dependencies
    if(THIS_DEPENDS)
    ...
    if(THIS_EXTERNAL_LIBS)
        if(BUILD_SHARED_LIBS)
            ...
            sfphysics_static_add_libraries(${target} ${THIS_EXTERNAL_LIBS})
        endif()
    endif()
...
macro(sfphysics_add_example target)
    ...
    # parse the arguments
    sfphysics_parse_arguments(THIS "SOURCES;DEPENDS" "GUI_APP" ${ARGN})
